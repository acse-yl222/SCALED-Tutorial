...
class ControlNet(ModelMixin):
    ...
    """
    def __init__(self, noise_latent_channels=128, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # multiple convolution layers
        self.conv_layers = nn.Sequential(
            nn.Conv3d(in_channels=1, out_channels=3, kernel_size=3, padding=1),
            nn.SiLU(),
        ...
        self.final_proj = nn.Conv3d(in_channels=128, out_channels=noise_latent_channels, kernel_size=1)
    ...
    def from_pretrained(cls, pretrained_model_path):
        ...
        model = ControlNet(noise_latent_channels=128)
...
class ControlNetv2(ModelMixin):
    ...
    def __init__(self, noise_latent_channels=320, *args, **kwargs):
        ...
        self.conv_layers = nn.Sequential(
            nn.Conv3d(in_channels=1, out_channels=3, kernel_size=3, padding=1),
            nn.SiLU(),
            nn.Conv3d(in_channels=3, out_channels=16, kernel_size=4, stride=2, padding=1),
            nn.SiLU(),
            nn.Conv3d(in_channels=16, out_channels=16, kernel_size=3, padding=1),
            nn.SiLU(),
    ...
    def from_pretrained(cls, pretrained_model_path):
        ...
        print(f"loaded PoseNet's pretrained weights from {pretrained_model_path}.")
        state_dict = torch.load(pretrained_model_path, map_location="cpu")
        model = ControlNetv2(noise_latent_channels=320)
        model.load_state_dict(state_dict, strict=True)
