...
# output
        ...
        if np.max(np.abs(w.cpu().detach().numpy())) > 80000.0:
            ...
            break
        if save_fig == True:
            result = np.stack([values_u.cpu().detach().numpy()[0,0,:,:].astype(np.float16),
                               values_v.cpu().detach().numpy()[0,0,:,:].astype(np.float16),
                               values_w.cpu().detach().numpy()[0,0,:,:].astype(np.float16)],axis=0)
            # 保存为 h5 文件
        with h5py.File(f"{save_path}/{itime:06d}.h5", "w") as f:
            f.create_dataset("uvw", data=result)
